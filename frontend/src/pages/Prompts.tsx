...
const Prompts = () => {
    ...
    const fetchHistory = async (name: string) => {
        try {
            const res = await api.get(`/prompts/${name}/history`);
            if (res.data) {
                setHistory(res.data);
                    ...
                    <div className="flex-1 bg-[#111827] rounded-xl border border-slate-800 flex flex-col overflow-hidden shadow-xl">
                        ...
                        <div className="flex border-b border-slate-800 bg-[#0D1117] px-6">
                            <button
                                ...
                                className={`px-8 py-3 text-sm font-medium border-b-2 transition-colors bg-black ${activeTab === 'editor' ? "border-teal-500 text-teal-300" : "border-transparent text-slate-500 hover:bg-slate-800 hover:text-white"}`}
                            >
                                Editor
                            </button>
                            <button
                                onClick={() => setActiveTab('config')}
                                className={`px-8 py-3 text-sm font-medium border-b-2 transition-colors bg-black ${activeTab === 'config' ? "border-teal-500 text-teal-300" : "border-transparent text-slate-500 hover:bg-slate-800 hover:text-white"}`}
                            >
                                Config
                            </button>
                            <button
                                onClick={() => setActiveTab('history')}
                                className={`px-8 py-3 text-sm font-medium border-b-2 transition-colors bg-black ${activeTab === 'history' ? "border-teal-500 text-teal-300" : "border-transparent text-slate-500 hover:bg-slate-800 hover:text-white"}`}
                            >
                                History
                            </button>
                        </div>
